...
def yolo_root_d4(param,yolo):
    ...
    return locate_d4(param,result) if result is not None else None
...
def yolo_recog(param, threshold, yolo):
    color_d435i = param.color_d435i
    
    # 将图像从 1280x720 缩放到 640x480
    resized_image = cv2.resize(color_d435i, (640, 360))  # (width, height)
    # 使用缩放后的图像进行预测
    detect_result = yolo.predict(resized_image,imgsz=640,half=False,max_det=1,verbose=False)       # 关闭调试输出)  # 用缩放后的图像做预测
    result = detect_result[0]  # 获取第一个结果（单图像输入）
    # 原始图像尺寸
    original_height, original_width = param.color_d435i.shape[:2]
    ...
    for box in result.boxes:
        ...
        conf = float(box.conf)
        param.logger.info(f"检测到 {name}，置信度：{conf:.4f}")
        ...
        if conf > threshold:
            ...
            x1_orig, y1_orig, x2_orig, y2_orig = map_to_original_coordinates(
                x1, y1, x2, y2, 
                original_width, original_height, 640, 360
            )
